...
* `triggers` - (Type: Object) Upstream triggers that would trigger the current sync to run.
  * `schedule` - (Type: Object) Schedule of the sync.
    ...
    * `cron_expression` - (Type: String) Cron expression for the sync's frequency. Ensure that for `frequency` above, the `expression` option is specified. Please refer to the Census [documentation](../core-concept/triggering-syncs.md#cron-custom-schedules) on what CRON expressions we support.
