...
* S3 account: You'll need an S3 bucket that's ready for use, along with credentials for an AWS user that can write to that bucket.
  * See AWS documentation for [setting up an S3 bucket](https://docs.aws.amazon.com/AmazonS3/latest/userguide/create-bucket-overview.html). And for&#x20;
...
* We support syncing files up to 100GB. Files larger than 5GB may require some additional permissions, see the
